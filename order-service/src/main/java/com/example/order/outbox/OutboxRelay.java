...
/**
 * Polls the outbox and relays pending events to Kafka. Runs on a fixed delay; a row
 * stays {@code PENDING} until the broker acks, so a crash mid-publish simply means it
 * is retried on the next tick. Re-publishing the same event is safe because consumers
 * dedupe on {@code eventId}.
 *
 * <p>For very high volumes, swap this poller for CDC (Debezium reading the H2/Postgres
 * WAL) to eliminate polling latency — the table contract stays identical.
 */
@Component
