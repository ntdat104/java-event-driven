...
import com.example.inventory.inbox.ProcessedEvent;
import com.example.inventory.inbox.ProcessedEventRepository;
...
/**
 * Crash-safe idempotency: Redis as a fast path (written only after commit, so a hit
 * always means "really processed"), DB inbox as the authoritative, concurrency-safe
 * source of truth (inserted in the caller's transaction). Call inside @Transactional.
 *
 * @return true if first time (process it); false if duplicate (skip + ack).
 */
@Service
